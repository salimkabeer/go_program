/*
Go has sort package which can be used for sorting the 
built-in as well as user defined data types.

The sort package has different methods to sort different data types
like Ints(), Float64s(), Strings() etc.

We can check if the values are sort or not by using the AreSorted() 
methods like Float64sAreSorted(), IntsAreSorted() etc.
