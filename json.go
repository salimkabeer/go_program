...
Boolean is converted to JSON booleans. Floating point, integer, and 
Number are converted to JSON numbers. The map's key type must either 
be a string, an integer type, or implement encoding.TextMarshaler.

The decoding of JSON is done using Unmarshal function.
Unmarshal function syntax is:
	"func Unmarshal(data []byte, v interface{}) error"  

	Unmarshal decodes JSON-encoded value and stores the result in the 
	value pointed to by v. If v is nil or not a pointer, Unmarshal 
	returns an InvalidUnmarshalError.

We can also customize the fields stored under the "json" key in the 
struct field's tag. we can have name of the field, followed by a 
comma-separated list of options. Like
	// The appears in JSON as key "myName".
